...
async def create_actuator(
    ...
    try:
        ...
        if not can_access_farm(user, Lab(**lab),'create'):
            raise HTTPException(
                status_code=403,
...
async def get_actuator_by_id(
    ...
    try:
        ...
        allowed = can_access_farm(user, Lab(**lab))
...
async def update_actuator(
    ...
    try:
        ...
        allowed = can_access_farm(user, Lab(**lab), 'update')
...
async def delete_actuator(
    ...
    try:
        ...
        allowed = can_access_farm(user, Lab(**lab), 'delete')
...
async def send_actuator_command(
    ...
    try:
        ...
        publish_task = asyncio.to_thread(
        ...
        ws_task = manager.send_update(
            actuatorId=actuator_status.actuator_id,
            value=actuator_status.value
        )

        publish_result, _, _ = await asyncio.gather(
            publish_task,
            db_task,
            ws_task,
        ...
        if not publish_result:
            return {"status": "failed", "detail": "Failed to publish MQTT message"}

        return {"status": "ok", "detail": "Actuator command sent successfully"}
...
async def get_actuator_status_by_actuator_id(
    ...
    try:
        ...
        allowed = await can_access_farm(user, Lab(**lab))
